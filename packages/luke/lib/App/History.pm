...
"targetRoot '$targetRoot' is not a valid value, must start with ./ or ../"
    ...
    if (!rindex $log_dir, "./", 0 and !rindex $log_dir, "../", 0) {
      croak(
        "log_dir '$log_dir' is not a valid value, must start with ./ or ../");
    }

    if (!-d -r -x -w $log_dir) {
      croak(
        "log_dir '$log_dir' has incompatible permissions or is not a directory."
      );
    }
    ...
    my $result = $input_dir->visit(
      sub {
        ...
        if ($path->is_file && "$path" =~ /\.mdwn$/ && -r $path) {
          ...
          my $newPath = join('/', $log_dir, $path->relative($input_dir));
          $newPath =~ s/\.mdwn$/.md/;
          ...
          foreach my $datum (@data) {
            ...
            my $loopindex = 0;
            while ($datum =~
/^\[\[!meta\h+?((date|updated|author|title|sortas){1}="(.+?)"(\h*?))+(\]\])?$/g
            ) {
              my $directive = $2;
              my $value     = $3;
              my $posVal    = pos($datum);
            ...
            while ($datum =~ /\[\[!img\h+?(\S+?)\h+?alt="(.+?)"\h*?\]\]/g) {
              ...
              my $imagePath = path(
                join(
                  "/", $assets, "log", $path->parent()->relative($input_dir)
                ),
                $img
              );
              ...
              $datum =~
s/\[\[!img\h+?(\S+?)\h+?alt="(.+?)"\h*?\]\]/![$2](\/$imagePath\/$1)/;
            }
   ...
   #I am not sure there *is* a reasonable match for raw wiki links with no path.
          ...
          $newPath->touch();
          my $newFH = $newPath->openw_utf8()
            or die "Could not open file '$newPath' $!";
          for (@frontmatter) {
            print $newFH "$_\n";
          }
          print $newFH "\n";
