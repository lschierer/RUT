...
class App::Build::ConversionManifest {
  ...
  my @CONVERSION_PATTERNS = (
    qr/\bfeat:\s*content\s+conversion\b/i, qr/\bfeat:\s*convert\s+content\b/i,
    qr/\bbuild:\s*format\s+conversion\b/i, qr/\bconvert\s+ikiwiki\b/i,
    qr/\bbulk\s+convert\b/i,               qr/\bauto-convert\b/i,
  );
  ...
  method analyze_all () {
    ...
    for my $file (sort @md_files) {
      ...
      my $output =
        eval { $repo->run('log', '--format=%H %s', '--follow', '--', $rel); };
      next unless $output;
