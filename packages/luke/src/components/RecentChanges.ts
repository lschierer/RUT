    ...
    for (const commit of commits) {
      ...
      for (const file of commit.files) {
        if (count > 100) {
          if (DEBUG) {
            console.log(
              `already found 100 pages for ${commit.id}. breaking to preserve resources`,
            );
          }
          break;
        }
      ...
      if (DEBUG) {
        console.log(
          `pushing entry with id ${commit.id} with ${pages.length} pages`,
        );
      }
      entries.push({
        id: commit.id,
